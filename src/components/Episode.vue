    ...
    <div class="bg-gray-900 text-gray-400 min-h-screen flex flex-col justify-between">
      <nav class="fixed top-0 left-0 w-full z-50 flex justify-between items-center bg-black border border-blue-700 p-4 h-25">
        ...
        <div class="flex space-x-4">
          ...
          <a target="_blank" href="https://github.com/RedietBT/Rick_and_Morty_API"><button class="rounded-xl border border-blue-700 text-blue-700 p-2 hover:scale-110 bg-white"><i class="fa-brands fa-github"></i></button></a>
          <RouterLink to="/">
        ...
        <header class="bg-gray-800 p-4 text-center">
            ...
            <h2 class="text-3xl font-bold mt-3">Episodes</h2>
        </header>
        <main class="p-4">
          <div class="container mx-auto p-4">
            ...
            <div v-else class="space-y-6">
              <div v-for="ep in result.episodes.results" :key="ep.id" class="bg-gray-800 shadow-md rounded-lg overflow-hidden transform transition-transform hover:scale-105 w-full flex flex-col md:flex-row mb-6">
                <div class="p-6 md:w-1/3 flex-shrink-0">
                  <RouterLink :to="'/episodes/' + ep.id">
                  <span class="font-semibold text-4xl text-blue-500">{{ ep.name }}</span><br><br>
                  </RouterLink>
                  <span class="font-semibold text-2xl">{{ ep.episode }}</span><br><br>
                  <span class="font-semibold text-2xl">({{ ep.air_date }})</span><br><br>
                  <span class="font-semibold text-2xl">({{ ep.created }})</span><br><br>
                </div>
                <div class="p-6 md:w-2/3 flex-shrink-0">
                  <h4 class="text-lg font-medium mb-2">Characters:</h4>
                  <div class="flex space-x-4 overflow-x-auto">
                    <div v-for="char in ep.characters" :key="char.id" class="min-w-max bg-gray-700 p-4 rounded-lg text-sm flex-shrink-0">
                      <RouterLink :to="'/characters/' + char.id">
                      <img :src="char.image" alt="Character Image" class="w-20 h-20 object-cover rounded-full border-4 border-blue-500/100">
                    </RouterLink>
                      <div class="p-4">
                        <h3 class="text-xl font-semibold mb-2 text-blue-500">{{ char.name }}</h3>
                        <span class="text-base font-semibold mb-2">{{ char.status }}</span><br>
                        <span class="text-base font-semibold mb-2">{{ char.species }}</span><br>
                        <span class="text-base font-semibold mb-2">{{ char.gender }}</span><br>
                      </div>
                    </div>
                  </div>
                </div>
